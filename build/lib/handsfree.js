(function (global, factory) {
  ...
  class HandposeModel extends BaseModel {
    ...
    updateMeshes(hand) {
      for (let i = 0; i < this.three.meshes.length - 1 /* palmbase */; i++) {
        ...
        this.normalized[i] = [this.handsfree.normalize(p0.x, this.handsfree.debug.$video.videoWidth / -2, this.handsfree.debug.$video.videoWidth / 2), this.handsfree.normalize(p0.y, this.handsfree.debug.$video.videoHeight / -2, this.handsfree.debug.$video.videoHeight / 2), this.three.meshes[i].position.z];
    ...
    webcam2space(x, y, z) {
      return new window.THREE.Vector3(x - this.handsfree.debug.$video.videoWidth / 2, -(y - this.handsfree.debug.$video.videoHeight / 2),
      // in threejs, +y is up
      -z);
    }
  ...
  class WebojiModel extends BaseModel {
    ...
    loadDependencies(callback) {
      ...
      this.loadDependency(`${this.handsfree.config.assetsPath}/jeeliz/jeelizFaceTransfer.js`, () => {
        ...
        fetch(url).then(model => model.json())
    ...
    getStates() {
      ...
      state.pursed = morphs[7] > this.handsfree.config.weboji.morphs.threshold.mouthRound;
      ...
      state.browsUpDown = state.browLeftDown && state.browRightUp || state.browRightDown && state.browLeftUp;
  ...
  var pluginFacePointer = {
    ...
    onFrame({
      ...
      y += z * Math.tan(weboji.rotation[0] + this.config.offset.pitch * Math.PI / 180) * this.config.speed.y - window.outerHeight;
  ...
  var pluginFaceClick = {
    ...
    onFrame({
      ...
      // Set the state
      if (this.mouseDowned > 0 && this.mouseDowned <= this.config.maxMouseDownedFrames) event = weboji.pointer.state = 'mousedown';else if (this.mouseDowned > this.config.maxMouseDownedFrames) event = weboji.pointer.state = 'mousedrag';else if (this.mouseUp) event = weboji.pointer.state = 'mouseup';else event = 'mousemove';
      ...
      if ($el) {
        ...
        // Focus
        if (weboji.pointer.state === 'mousedown' && ['INPUT', 'TEXTAREA', 'BUTTON', 'A'].includes($el.nodeName)) $el.focus();
